	...
	for _, group := range comments {
		for _, comment := range group.List {
			...
			case strings.HasPrefix(comment.Text, "//go:cgo_import_dynamic "),
				...
				fields[1] = tf.transformCgoImportLocalName(fields[1])
				comment.Text = strings.Join(fields, " ")
			}
		}
	}
...
func (tf *transformer) transformDirectiveLocalName(localName string) string {
...
func (tf *transformer) transformCgoImportLocalName(localName string) string {
	if localNamePart, ok := strings.CutPrefix(localName, tf.curPkg.ImportPath+"."); ok {
		return tf.curPkg.obfuscatedImportPath() + "." + tf.transformDirectiveLocalName(localNamePart)
	}
	return localName
}

func (tf *transformer) transformLinkname(localName, newName string) (string, string) {
	localName = tf.transformDirectiveLocalName(localName)
	if newName == "" {
		return localName, ""
	}
